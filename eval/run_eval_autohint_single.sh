#!/usr/bin/env bash
#
# run_eval_autohint_single.sh -- evaluate ONE checkpoint of the AutoHint run
#   HPRL-AutoHint-Qwen2.5-7B-dapo-20260622-200915
# on the 3 eval datasets, in the SAME format as the usual GRPO global_step_200
# case: BARE single-turn, NO --hint-mode, box-scored with mathruler (directly
# comparable to baseline / GRPO numbers).
#
# The verl FSDP actor is merged to HF (model_merger), served once on a local
# vLLM OpenAI endpoint (DP=8, TP=1), and eval_ckpts.py rolls every problem out
# N times (default 128) and grades the boxed answer.
#
...
#   STEP                 which global_step to eval (default: latest = 80)
...
#   AUTOHINT_CKPT        full path to the actor dir (overrides STEP)
# ---------------------------------------------------------------------------
set -eo pipefail
...
# --- the AutoHint checkpoint to evaluate -----------------------------------
RUN_NAME="HPRL-AutoHint-Qwen2.5-7B-dapo-20260625-230822"
RUN_CKPT_DIR="$HINT_RL_HOME/ckpt/HPRL-AutoHint-Qwen2.5-7B-Instruct/$RUN_NAME"
STEP="${STEP:-40}"
AUTOHINT_CKPT="${AUTOHINT_CKPT:-$RUN_CKPT_DIR/global_step_${STEP}/actor}"
LABEL="autohint-0622-step${STEP}"
SERVED_NAME="autohint-step${STEP}"
...
OUT_BASE="${OUT_BASE:-$SCRIPT_DIR/results/autohint_${RUN_TS}}"
...
[ -d "$AUTOHINT_CKPT" ] || { echo "FATAL: actor dir not found: $AUTOHINT_CKPT (set STEP or AUTOHINT_CKPT)" >&2; exit 1; }
...
echo ""
echo "##################################################################"
echo "# MODEL: $LABEL   ($AUTOHINT_CKPT)"
echo "##################################################################"
HF_DIR="$(merge_fsdp "$AUTOHINT_CKPT" "$MERGED_DIR/${LABEL}")"
