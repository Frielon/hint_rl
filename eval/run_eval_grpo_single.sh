#!/usr/bin/env bash
#
# run_eval_grpo_single.sh -- evaluate ONE checkpoint of the GRPO run
#   GRPO-Qwen2.5-7B-Instruct-dapo-512-20260624-201758
# on the 3 eval datasets: BARE single-turn, NO --hint-mode, box-scored with
# mathruler (directly comparable to baseline / AutoHint numbers).
#
...
#   STEP                 which global_step to eval (default: 180)
...
#   GRPO_CKPT            full path to the actor dir (overrides STEP)
# ---------------------------------------------------------------------------
set -eo pipefail
...
# --- the GRPO checkpoint to evaluate ---------------------------------------
RUN_NAME="GRPO-Qwen2.5-7B-Instruct-dapo-512-20260624-201758"
RUN_CKPT_DIR="$HINT_RL_HOME/ckpt/GRPO-Qwen2.5-7B-Instruct/$RUN_NAME"
STEP="${STEP:-40}"
GRPO_CKPT="${GRPO_CKPT:-$RUN_CKPT_DIR/global_step_${STEP}/actor}"
LABEL="grpo-0624-step${STEP}"
SERVED_NAME="grpo-step${STEP}"
...
OUT_BASE="${OUT_BASE:-$SCRIPT_DIR/results/grpo_${RUN_TS}}"
...
[ -d "$GRPO_CKPT" ] || { echo "FATAL: actor dir not found: $GRPO_CKPT (set STEP or GRPO_CKPT)" >&2; exit 1; }
...
echo ""
echo "##################################################################"
echo "# MODEL: $LABEL   ($GRPO_CKPT)"
echo "##################################################################"
HF_DIR="$(merge_fsdp "$GRPO_CKPT" "$MERGED_DIR/${LABEL}")"
