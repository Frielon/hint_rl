...
if [ ! -f "${MODEL_PATH}/config.json" ]; then
    echo "[run_auto_hint_qwen3_4b_instruct_async_restart] ERROR: model dir not found: ${MODEL_PATH}" >&2
    exit 1
fi
...
QWEN3_4B_S100_CKPT="${HINT_RL_HOME}/ckpt/HPRL-AutoHint-Qwen3-4B-Instruct-2507-async/HPRL-AutoHint-Qwen3-4B-Instruct-2507-async-dolci-instruct-492-20260802-000856/global_step_100"
export RESUME_FROM_PATH=${RESUME_FROM_PATH-}
if [ -n "${RESUME_FROM_PATH}" ] && [ ! -d "${RESUME_FROM_PATH}/actor" ]; then
    echo "[run_auto_hint_qwen3_4b_instruct_async_restart] ERROR: RESUME_FROM_PATH has no actor/ ckpt: ${RESUME_FROM_PATH}" >&2
    exit 1
fi
...
# --- RESTART DELTA 3: distinct run labels -----------------------------åååå---------
export project_name=${project_name:-'HPRL-AutoHint-Qwen3-4B-Instruct-2507-async'}
# export exp_name=${exp_name:-"HPRL-AutoHint-Qwen3-4B-Instruct-2507-async-restart-dolci-instruct-492-$(TZ='America/Los_Angeles' date +%Y%m%d-%H%M%S)"}
export exp_name=${exp_name:-"HPRL-AutoHint-Qwen3-4B-Instruct-2507-async-restart-dolci-instruct-492-20260804-233932"}

echo "[run_auto_hint_qwen3_4b_instruct_async_restart] TERMINATE-AND-RESTART auto-hint mode ON  (model=${MODEL_PATH})"
echo "[run_auto_hint_qwen3_4b_instruct_async_restart]   agent registry: ${AGENT_LOOP_CONFIG_PATH} (auto_hint -> RestartHintAgentLoop)"
echo "[run_auto_hint_qwen3_4b_instruct_async_restart]   context: prompt=${max_prompt_length} response=${max_response_length} (262144 native) per-segment cap=${HPRL_MAX_TURN_TOKENS} (live context <= $((max_prompt_length + HPRL_MAX_TURN_TOKENS)) tok)"
echo "[run_auto_hint_qwen3_4b_instruct_async_restart]   TRAIN_FILE=${TRAIN_FILE}"
echo "[run_auto_hint_qwen3_4b_instruct_async_restart]   resume_from=${RESUME_FROM_PATH:-<fresh>}"
echo "[run_auto_hint_qwen3_4b_instruct_async_restart]   split trainer:rollout=${TRAINER_NNODES}:${ROLLOUT_NNODES} nodes  sp=${SP_SIZE} ppo_token_mult=${PPO_TOKEN_MULT}  staleness=${STALENESS_THRESHOLD} trigger=${TRIGGER_PARAMETER_SYNC_STEP} partial_rollout=${PARTIAL_ROLLOUT} rollout_corr_bypass=${ROLLOUT_CORR_BYPASS}"
echo "[run_auto_hint_qwen3_4b_instruct_async_restart]   strategy=${HINT_STRATEGY} ratchet=${HPRL_RATCHET_MODE} allow_decrease=${HPRL_ALLOW_DECREASE} progress_message=${HPRL_AUTO_HINT_PROGRESS_MESSAGE} pool_wording=${HPRL_RESTART_POOL_WORDING} train_segments=${HPRL_RESTART_TRAIN_SEGMENTS}"
echo "[run_auto_hint_qwen3_4b_instruct_async_restart]   step_adv=${HPRL_STEP_ADV} whole_turn=${HPRL_STEP_ADV_WHOLE_TURN} norm=${HPRL_STEP_ADV_NORM} overlong=${HPRL_OVERLONG_PENALTY}(${HPRL_OVERLONG_PENALTY_TYPE}) -- final-segment rows, phantom fail records keep the value fit chain-exact"
