#!/usr/bin/env bash
#
# AUTO-HINT (push-hint) HPRL run for Qwen3-4B-Instruct-2507 -- FULLY-ASYNC
# edition, on the hinted Dolci-Instruct-RL set.
#
# Cloned from run_auto_hint_olmo3_7b_instruct_dpo_async.sh (same async
# architecture, reward terms, step-level advantage config incl. its
# value-routed overlong penalty, ratchet policy); ALL training params are kept
# identical -- the only delta is the model:
#
#   1. model -- ${BASE_HOME}/model/Qwen3-4B-Instruct-2507, used AS-IS: being an
#      instruct checkpoint it already closes turns properly --
#      generation_config.json ships eos [151645 <|im_end|>, 151643
#      <|endoftext|>] -- and the chat template lives in tokenizer_config.json,
#      which transformers auto-loads; the agent-loop init that crashed on
#      template-less BASE checkpoints is fine here.
#   2. context -- 262144 NATIVE (max_position_embeddings, no YaRN needed), so
#      the requested max_response_length=34768 fits with huge margin:
#      prompt(2048)+response = 36816 << 262144.
#   3. per-turn cap -- HPRL_MAX_TURN_TOKENS=16384, unchanged from the Olmo DPO
#      wrapper: a single turn may spend up to 16384 tokens, the whole rollout
#      up to 34768 across turns.
...
#   STALENESS_THRESHOLD=2 TRAINER_NNODES=1 bash run_auto_hint_qwen3_4b_instruct_2507_async.sh
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
HINT_RL_HOME=${HINT_RL_HOME:-"$(cd "${SCRIPT_DIR}/../.." && pwd)"}
BASE_HOME=${BASE_HOME:-"$(cd "${HINT_RL_HOME}/../.." && pwd)"}

# --- base model: Qwen3-4B-Instruct-2507, as-is (see header pt. 1) --------------
export MODEL_PATH=${MODEL_PATH:-"${BASE_HOME}/model/Qwen3-4B-Instruct-2507"}
if [ ! -f "${MODEL_PATH}/config.json" ]; then
    echo "[run_auto_hint_qwen3_4b_instruct_async] ERROR: model dir not found: ${MODEL_PATH}" >&2
    exit 1
fi

# --- context budget: 34768 fits the 262144 native context (header pt. 2) -------
export max_response_length=${max_response_length:-50768}
...
export project_name=${project_name:-'HPRL-AutoHint-Qwen3-4B-Instruct-2507-async'}
# export exp_name=${exp_name:-"HPRL-AutoHint-Qwen3-4B-Instruct-2507-async-dolci-instruct-492-$(TZ='America/Los_Angeles' date +%Y%m%d-%H%M%S)"}
export exp_name=${exp_name:-"HPRL-AutoHint-Qwen3-4B-Instruct-2507-async-dolci-instruct-492-20260802-000856"}

echo "[run_auto_hint_qwen3_4b_instruct_async] auto-hint FULLY-ASYNC mode ON  (model=${MODEL_PATH})"
echo "[run_auto_hint_qwen3_4b_instruct_async]   context: max_response_length=${max_response_length} (262144 native) max_turn_tokens=${HPRL_MAX_TURN_TOKENS}"
echo "[run_auto_hint_qwen3_4b_instruct_async]   TRAIN_FILE=${TRAIN_FILE}"
echo "[run_auto_hint_qwen3_4b_instruct_async]   resume_from=${RESUME_FROM_PATH:-<fresh>}"
echo "[run_auto_hint_qwen3_4b_instruct_async]   split trainer:rollout=${TRAINER_NNODES}:${ROLLOUT_NNODES} nodes  staleness=${STALENESS_THRESHOLD} trigger=${TRIGGER_PARAMETER_SYNC_STEP} require=${REQUIRE_BATCHES} partial_rollout=${PARTIAL_ROLLOUT} rollout_corr_bypass=${ROLLOUT_CORR_BYPASS}"
echo "[run_auto_hint_qwen3_4b_instruct_async]   strategy=${HINT_STRATEGY} call_reward=${HINT_CALL_REWARD} shape=${HINT_SHAPE_COEFF} ratchet=${HPRL_RATCHET_MODE} allow_decrease=${HPRL_ALLOW_DECREASE}"
echo "[run_auto_hint_qwen3_4b_instruct_async]   step_adv=${HPRL_STEP_ADV} step_adv_scale=${HPRL_STEP_ADV_SCALE} step_adv_norm=${HPRL_STEP_ADV_NORM} whole_turn=${HPRL_STEP_ADV_WHOLE_TURN} overlong_penalty=${HPRL_OVERLONG_PENALTY} overlong_type=${HPRL_OVERLONG_PENALTY_TYPE} guidance_free=${HINT_GUIDANCE_FREE} prune_guidance=${HPRL_PRUNE_GUIDANCE} progress_message=${HPRL_AUTO_HINT_PROGRESS_MESSAGE} lr_sched=${HPRL_LR_SCHEDULER}"
