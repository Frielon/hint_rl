...
#
# START / RESUME:
#   default            FRESH run from the pristine base model: idempotently
#                      (re)build the eos-derived ${BASE_HOME}/model/
#                      Qwen3-8B-Base-hprl dir -- per-file symlinks into the
#                      pristine dir + ONE real file, the patched
#                      generation_config.json with eos [151643,151645] --
#                      exactly the sync wrapper's block. RESUME_FROM_PATH is
#                      empty -> trainer.resume_mode=auto, which still picks up
#                      THIS experiment's own latest ckpt if the same exp_name
#                      is relaunched.
#   RESUME_FROM_PATH=  continue optimizer+dataloader from a .../global_step_N
#                      dir. Constraints: (a) the TRAINER pool must match the
...
# raise-only adaptive ratchet re-fits online). To carry a prior run's ratcheted
# budgets, point BUDGET_STATE_PATH at a COPY of that run's budget_state.json
# (the trainer writes to it -- never share the original).
#
...
export exp_name=${exp_name:-"HPRL-AutoHint-Qwen3-8B-Base-async-dolci-rl-zero-10324-$(TZ='America/Los_Angeles' date +%Y%m%d-%H%M%S)"}
