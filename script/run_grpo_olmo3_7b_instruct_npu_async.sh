...
# ---------------------------------------------------------------------------
# FULLY-ASYNC (disaggregated) variant of run_grpo_olmo3_7b_instruct_npu.sh:
# plain single-turn GRPO on Olmo-3-7B-Instruct-DPO, but instead of the
# colocated synchronous trainer (generate -> update -> generate on the same
...
#     column) route through verl's built-in single_turn_agent, and the reward
#     (custom_reward.py) is computed streaming on this pool by the reward
#     loop -- ground truth never needs to reach the trainer.
...
# (verl.experimental.fully_async_policy.fully_async_main). No HPRL machinery:
# this is the plain GRPO run, so the hint agent loop / budget ratchet /
# step-adv -- and the HPRLFullyAsyncRollouter non-tensor restore only they
# need -- are all irrelevant here.
#
...
#   * Equivalence bookkeeping: REQUIRE_BATCHES(1) x ppo_mini(64) x TRIGGER(1)
#     = 64 prompts per weight sync == one sync step (train_prompt_bsz=64), so
#     a "param version" here corresponds 1:1 to a sync global step and
#     test_freq / save_freq / total_epochs keep their sync meanings.
#   * Off-policy handling: rollout.calculate_log_probs=True +
#     actor.use_rollout_log_probs=True + rollout_correction.bypass_mode=True
#     (the stock fully-async defaults): PPO clips against the behavior
#     policy's sampling-time logprobs. ROLLOUT_CORR_BYPASS=False (decoupled
#     recompute) additionally requires ACTOR_STRATEGY=fsdp2 on the STOCK
...
#
# Expected tree layout and path derivation are identical to the sync script;
...
if [ "${TRAINER_NNODES}" -lt 1 ] || [ "${ROLLOUT_NNODES}" -lt 1 ]; then
    echo "[run_grpo_olmo3_async] FATAL: need >=1 trainer node and >=1 rollout node" \
         "(got trainer=${TRAINER_NNODES} rollout=${ROLLOUT_NNODES} of NNODES=${NNODES})." >&2
    exit 1
fi
if [ "$((TRAINER_NNODES + ROLLOUT_NNODES))" -gt "${NNODES}" ]; then
    echo "[run_grpo_olmo3_async] FATAL: TRAINER_NNODES(${TRAINER_NNODES}) + ROLLOUT_NNODES(${ROLLOUT_NNODES})" \
...
if [ "${ROLLOUT_CORR_BYPASS}" != "True" ] && [ "${ACTOR_STRATEGY}" != "fsdp2" ]; then
    echo "[run_grpo_olmo3_async] FATAL: ROLLOUT_CORR_BYPASS=${ROLLOUT_CORR_BYPASS} needs" \
...
# Data: identical to the sync script -- single-turn dolci prompts (no
# agent_name column -> verl's built-in single_turn_agent) + the 4 bare val sets.
# TRAIN_FILE=${TRAIN_FILE:-"${HINT_RL_HOME}/dataset/dolci-dapo-2180-single-turn.parquet"}
TRAIN_FILE=${TRAIN_FILE:-"${HINT_RL_HOME}/dataset/dolci-instruct-rl-492-auto-hint-qwen3-4b-le1of8-single-turn.parquet"}
...
# ray_cluster_launch_olmo3_7b_instruct_async.sh, which brings up Ray on every
# pod first and then execs this script on the head.
if ! python3 -c 'import sys, urllib.request as u; u.urlopen(sys.argv[1].rstrip("/") + "/api/version", timeout=10)' "${RAY_ADDRESS}" 2>/dev/null; then
    echo "[run_grpo_olmo3_async] FATAL: no Ray cluster answering at ${RAY_ADDRESS}." >&2
    echo "  This script only submits the training job to an EXISTING Ray cluster." >&2
    echo "  On the training platform, submit ray_cluster_launch_olmo3_7b_instruct_async.sh" >&2
