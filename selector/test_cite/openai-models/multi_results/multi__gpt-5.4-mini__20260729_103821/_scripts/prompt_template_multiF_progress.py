...
# hint rephrased to fit the student's actual reasoning trace (same rephrasing
# treatment the selected hint gets in step 3, but stated as progress already
# made, in the student's own notation). Downstream, `progress` is what gets
# surfaced to the policy model to tell it which steps it has already earned;
# `quote`/`why` stay untouched so citation verification keeps working.
#
...
achieved hint is also rephrased -- adapted to the student's trace -- into a
``progress`` statement meant to be shown to the student.
...
TEMPLATE_MULTI_F = r"""You are an expert math tutor helping a student over multiple rounds. In earlier rounds some hints were already given to the student and verified as achieved; those are marked `status: "completed"`. Your task is to choose the first hint that the student has not yet achieved, considering only the hints still marked `status: "pending"`, based on the student's reasoning trace. For every pending hint the student newly achieved this round, you also restate it as progress already made, adapted to the student's own reasoning, so the student can be told what they have accomplished.
...
5. Rephrase each completed hint as achieved progress:
   ...
   * The `progress` field will be shown to the student to tell them what they have already accomplished. Rephrase the hint to accommodate the student's reasoning trace — the same kind of rephrasing as step 3, but describing progress already made rather than what to do next.
   * Keep the mathematical intent of the original hint unchanged.
   * State it as something already done (e.g., "You have already established that ..."), never as an instruction still to be carried out.
   * Adapt it to the student's actual work: reuse the student's own notation, variable names, and line of approach from the trace where they differ from the hint's wording.
   * Do not reveal solution content beyond what the original hint already contains.
   * If the original hint already matches the student's work, a minimal rewording into achieved-progress form is enough.
...
"progress": "<the hint rephrased per step 5: a statement of the progress the student has already made, adapted to the student's own notation and approach — this is shown to the student>"
