...
def fmt(runs: list[dict]) -> str:
    ...
    L.append("\n### Token usage & estimated cost\n")
    L.append("| model | calls | in_tok | out_tok | total_tok | est_cost |")
    L.append("|---|--:|--:|--:|--:|--:|")
    total = 0.0
    for r in runs:
        s = r["summary"]
    ...
    L.append("\n_est_cost is an estimate (prompt_tokens at input price — ignores any cache "
             "discount; completion_tokens at output price, incl. reasoning tokens). Runs missing "
             "a `usage` block show `—`; back-fill with `python recost.py`. Prices live in "
             "`pricing.py` (edit or set `OPENAI_PRICES_JSON`)._")
