...
def _repair_json_escapes(s: str) -> str:
    ...
    The stray-case replacement is TWO backslash characters (the lambda return
    is literal -- no replacement-escape processing), so `\(` becomes `\\(` and
    decodes back to `\(`. The old 4-char return over-doubled strays, so
    decoded text mixed `\\(` with `\frac`."""
...
def _hard_parse(text: str):
    ...
    # and usually precede the selection fields, so a first-match grab returns a
    # cited (already-achieved) id. The selected id sits next to the top-level
    # "hint"/"reasoning_of_hint" keys; prefer the match nearest to those.
    ids = list(re.finditer(r'"hint_id"\s*:\s*"?(\d+(?:\.\d+)?)"?', text))
    if ids:
        anchor = re.search(r'"(?:hint|reasoning_of_hint)"\s*:\s*"', text)
        best = min(ids, key=lambda i: abs(i.start() - anchor.start())) if anchor else ids[-1]
...
def _is_citation_item(d: dict) -> bool:
    ...
    hint_id, so without this guard it becomes a selection with empty text."""
...
def _as_selection_dict(obj):
    ...
    if isinstance(obj, dict):
        return None if _is_citation_item(obj) else obj
